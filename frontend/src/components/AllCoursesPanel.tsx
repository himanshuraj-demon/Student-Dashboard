...
function SelectableCourseRow({
  ...
  return (
    <li
      ...
      <div className="flex items-center gap-2.5 min-w-0 flex-1">
        <span
          className={`inline-block w-4 h-4 rounded border-2 shrink-0 flex items-center justify-center
          ${
            selected
...
interface AllCoursesPanelProps {
  ...
  handleToggleCourse: (code: string) => void;
    ...
    return (
      <main className="flex-1 min-w-0 overflow-hidden flex flex-col gap-4">
        ...
        <div className="bg-[#ffffff11] branchpanelsearch rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <h1 className="text-xl font-bold ">Add Courses</h1>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="flex-1 sm:flex-none px-4 py-2 text-sm font-semibold rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
                Cancel
              </button>
              <button
                onClick={handleSaveChanges}
                disabled={isSaving}
                className="flex-1 sm:flex-none px-4 py-2 text-sm font-semibold rounded-xl bg-violet-600 text-white hover:bg-violet-700 disabled:opacity-60 transition-colors flex items-center justify-center gap-1.5">
                {isSaving ? (
                  <>
                    <svg
        ...
        <div className="relative w-full">
          ...
          <input
            className="w-full pl-9 pr-8 py-2.5 text-sm rounded-xl border  border-gray-200 branchpanelsearch bg-[#ffffff11] shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-300 placeholder:text-gray-400"
        ...
        <section className="branchpanelsearch rounded-2xl border border-gray-100 shadow-sm overflow-x-hidden min-w-0">
          <div className="flex items-center justify-between px-4 sm:px-5 py-3 border-b border-gray-50">
            ...
            <span className="text-xs bg-violet-100 text-violet-700 font-bold px-2 py-0.5 rounded-full shrink-0">
              {allCoursesFiltered.length} courses
            </span>
          </div>
          <ul className="flex flex-col gap-1.5 p-3 max-h-[60vh] md:overflow-y-scroll overflow-x-hidden">
      ...
      <div className="sticky top-0 z-20 bg-[#ffffff11] branchpanelsearch rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5 backdrop-blur-sm">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="shrink-0 flex items-center gap-1.5 px-3 sm:px-4 py-2 text-sm font-semibold rounded-xl bg-violet-600 text-white hover:bg-violet-700 transition-colors">
            <svg
            ...
            <span className="hidden xs:inline sm:inline">Add Courses</span>

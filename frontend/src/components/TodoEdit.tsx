...
const availableTags = ["coding", "game", "study", "music", "fitness", "personal"];

const TodoEdit = ({ todo, setEdit }: Props) => {
  const { register, handleSubmit, reset, setError, formState: { errors } } = useForm<Todo>();
  const { setTodos } = useAuth();
  const [selectedTag, setSelectedTag] = useState<string>(todo?.tag || "personal");
  ...
  useEffect(() => {
    if (todo) {
      ...
      const formattedTodo = {
        ...todo,
        time: todo.time ? new Date(todo.time).toISOString().substring(0, 16) : ""
      };
      reset(formattedTodo);
    }
  }, [todo, reset]);
    ...
    <>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-70 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-[#161b33] border border-slate-800 text-white rounded-2xl p-6 w-[90%] max-w-sm flex flex-col gap-4 shadow-2xl">
            <h2 className="text-xl font-bold text-center">Discard Task Status?</h2>
            <p className="text-center text-slate-400 text-sm">
              This completely purges historical timeline visibility logs. This action cannot be undone.
            </p>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs">
        <div className="w-full h-full md:w-125 md:h-auto md:max-h-[90vh] md:rounded-2xl bg-[#161b33] border border-slate-800 text-white shadow-2xl flex flex-col overflow-y-auto">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col p-5 gap-5">
            
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-400">Task Title</label>
              <input
                {...register("title", { required: "A descriptive title assignment is required" })}
              ...
              />
              {errors.title && <span className="text-red-400 text-xs mt-0.5">{errors.title.message}</span>}
            </div>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-400">Category Tag</label>
